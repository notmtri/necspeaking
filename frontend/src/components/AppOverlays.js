...
export function AdminLoginModal({ open, password, onPasswordChange, onClose, onSubmit, error, submitting }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/75 p-4 backdrop-blur-sm">
      <div
        className="w-full max-w-md rounded-[24px] border border-white/10 bg-[#081120] p-5 shadow-[0_30px_120px_rgba(2,6,23,0.55)] sm:rounded-[28px] sm:p-6"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h2 id="admin-login-title" className="mt-2 text-2xl font-black text-white">Enter admin password</h2>
          </div>
          <button type="button" onClick={onClose} disabled={submitting} className="rounded-full border border-white/10 bg-white/[0.04] p-2 text-slate-300 transition hover:bg-white/[0.08] hover:text-white disabled:cursor-not-allowed disabled:opacity-60" aria-label="Close admin login">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="mt-5 space-y-4">
          <input
            ...
            autoFocus
            className="w-full rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-white outline-none transition placeholder:text-slate-400 focus:border-sky-400/50"
          />
          {error && (
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm">
      ...
      >
        ...
        <h3 id="confirm-modal-title" className="mt-4 text-2xl font-black text-white">{title}</h3>
  ...
  return (
    <div className="pointer-events-none fixed bottom-4 left-4 right-4 z-[90] flex w-auto max-w-none flex-col gap-3 sm:left-auto sm:w-full sm:max-w-sm">
      {toasts.map((toast) => {
        ...
        return (
          <div key={toast.id} className={`pointer-events-auto rounded-2xl border px-4 py-3 shadow-[0_20px_60px_rgba(2,6,23,0.4)] backdrop-blur ${toneClasses}`} role={toast.tone === 'error' ? 'alert' : 'status'} aria-live={toast.tone === 'error' ? 'assertive' : 'polite'}>
            <div className="flex items-start gap-3">
              ...
              <button type="button" onClick={() => dismissToast(toast.id)} className="rounded-full border border-white/10 bg-white/[0.04] p-1 text-current/80 transition hover:bg-white/[0.08] hover:text-current" aria-label="Dismiss notification">
                <X size={14} />
              </button>
            </div>
          </div>
